...
void main() {
  ...
  group('State transitions', () {
    ...
    test(
      'after run: loading=true → success: loading=false, data=result, error=null',
      () async {
    ...
    test(
      'after failed run: loading=true → error: loading=false, data=null, error=exception',
      () async {
        final completer = Completer<String>();
        final notifier = UseRequestNotifier<String, int>(
          service: (p) => completer.future,
    ...
    test(
      'after error then successful refresh: error clears, data updates',
      () async {
        var shouldFail = true;
        final notifier = UseRequestNotifier<String, int>(
          service: (p) async {
    ...
    test('initialData: initial state has data=initialData', () {
      final notifier = UseRequestNotifier<String, int>(
        service: (p) async => 'result-$p',
        options: const UseRequestOptions(manual: true, initialData: 'initial'),
      );
    ...
    test(
      'mutate to null clears cache entry to keep state/cache consistent',
      () async {
        final notifier = UseRequestNotifier<String, int>(
          service: (p) async => 'result-$p',
          options: UseRequestOptions(
            manual: true,
            cacheKey: (p) => 'mutate-null-$p',
          ),
        );

        await notifier.runAsync(3);
        expect(RequestCache.get<String>('mutate-null-3')?.data, 'result-3');

        notifier.mutate((old) => null);
        expect(notifier.currentState.data, isNull);
        expect(RequestCache.get<String>('mutate-null-3'), isNull);
    ...
    test(
      'loadMore: loadingMore=true during request, loading stays false',
      () async {
